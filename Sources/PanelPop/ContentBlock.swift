...
//
//   let contentBlocks = try? JSONDecoder().decode(ContentBlocks.self, from: jsonData)
...
struct ContentBlocks: Codable, Sendable {
    let time: Int
    let blocks: [Block]
    let version: String
}
...
struct DataClass: Codable, Sendable {
    ...
    let file: File?
    let caption: String?
    let stretched, withBorder, withBackground: Bool?
}
...
struct ItemMeta: Codable, Sendable {
    let checked: Bool?
}

// MARK: - DataMeta

struct DataMeta: Codable, Sendable {
